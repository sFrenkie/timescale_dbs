...
begin
  ...
  HOUSES.times do |id|
    ...
    con.exec("CREATE TABLE same_value_#{id}(id bigserial, sensor_id SMALLINT, value REAL, timestamp TIMESTAMPTZ NOT NULL)")
    con.exec("SELECT create_hypertable('same_value_#{id}', 'timestamp')")
  end
  con&.close
